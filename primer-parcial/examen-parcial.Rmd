...
---
Integrantes : Yalidt Díaz 141394 y Bruno Gonzalez 
...
```{r, cache = T}
#Si es con lineup? o mas bien con un rerun??
permutaciones <- lineup(null_permute("aerosol"), 
                        truenos, n = 10000)
```

```{r}
permutaciones %>% group_by(.sample,aerosol) %>%
  summarise(total_orden=sum(orden), .groups = 'drop') %>%
...
**5\% extra.** Acontinuación mostramos lo que una prueba vista en cursos clásicos de
