...
tibble(t=theta) %>% 
  ggplot(aes(x=t)) +
    geom_histogram(bins = 35) + ggtitle("Distribución de referencia")
```




**R. Se obtiene un histograma que esta sesgado a la derecha y que hay algunos datos atípicos para valores mayores a 1 de $\theta$, esto nos diría que hay muy pocas ocasiones en las que la $\lambda_s$ es mayor a $\lambda_n$ (hay mayor concentración de los datos en las tormentas que no se les aplica el aerosol). Por otra parte se observa que hay bastantes observaciones con $\theta$ entre aproximadamente .01 y .02 (aqui el cociente de las lambdas es muy bajo), por lo que en este rango de thetas hay mayor(menor no?) dispersión en las tormentas que se les aplica el aerosol. Por último se observa que la distribución parece ser bimodal, es decir que representa justamente los 2 grupos de tormentas**

10. Cuál es tu conjetura de que la distribución de referencia tenga esa forma?

**R. Que los grupos de tormentas estan separados y tienen distribuciones distintas, se ven claramente 2 modas en el histograma obtenido. Y como primer creencia se tendría que las thetas pequeñas son aquellas donde hay mayor dispersión en las tormentas sometidas (sin no?)al aerosol, y por el contrario las thetas entre .4 y .9 las tormetas que tienen mayor dispersión son las que no se someten al aerosol**
