...
**R. El total de posibilidades que se podrían obtener serían 1352078, porque hay 23 órdenes, las cuales se dividen en 12 (o 11) posibles valores Si (o No)**
...
```{r 2.9 Valor-p de la prueba de hipótesis}
...
```{r 3.3 Histograma Distribucion de referencia con permutaciones sobre conteos}

tibble(t=theta) %>% 
  ggplot(aes(x=t)) +
    geom_histogram(bins = 35) + ggtitle("Distribución de referencia permutaciones")
```
...
**R. Que los grupos de tormentas estan separados y tienen distribuciones distintas, se ven claramente 2 modas en la gráfica del histograma obtenido. Y como primer creencia se tendría que las thetas pequeñas son aquellas donde hay mayor dispersión en las tormentas sometidas (sin no?)al aerosol, por el contrario  en el intervalo de thetas (2 - 6) las tormetas que tienen mayor dispersión son las que no se someten al aerosol. Esto justamente comprueba lo de la prueba de hipótesis anterior en donde rechazamos $H_0$ y considerabamos que efectivamente había una diferencia entre las tormetas que reciben o no el aerosol**
...
```{r 3.5 Simulaciones de boostrap}

set.seed(4020)
simulaciones <- rerun(1000,funcion_bootstrap(truenos)) %>% flatten_dbl()
```

```{r 3.6 Histograma de las simulaciones}

tibble(x=simulaciones) %>% 
  ggplot(aes(x=x)) +
    geom_histogram(bins = 40) + ggtitle("Distribución de referencia boostrap")
```

**R. Se obtiene un histograma la escala de theta cambia a numeros de aproximadamente entre (0 y 1), además se observa que esta sesgado a la derecha y que hay algunos datos atípicos para valores mayores a 1 de $\theta$, esto nos diría que hay muy pocas ocasiones en las que la $\lambda_s$ es mayor a $\lambda_n$ (hay mayor concentración de los datos en las tormentas que no se les aplica el aerosol). Por otra parte se observa que hay bastantes observaciones con $\theta$ entre aproximadamente .01 y .02 (aqui el cociente de las lambdas es muy bajo), por lo que en este rango de thetas hay menor dispersión en las tormentas que se les aplica el aerosol. Por último se observa que la distribución parece ser bimodal, es decir que representa justamente los 2 grupos de tormentas**

12. Reporta la dispersión de tu estimador en forma de intervalos con el método 
más adecuado de los vistos en clase. 


```{r 3.7 Intervalos de theta}
#Librería rsample
library(rsample)
...
# term (que es el nombre del estimador)
  truenos <- analysis(split)   # ¿Si va truenos o una muestra?

  truenos %>% summarise(estimate = funcion_bootstrap(.)) %>% mutate(term = "theta") 

}

thetas_boot <- bootstraps(truenos, strata = aerosol, 1000) %>%
  mutate(boot_e=map(splits, estimador_boot))

#Calculo de intervalos al 95% del estimador theta
thetas_boot %>% int_pctl(boot_e, alpha= .05)
```

13. Dado el estimador y los intervalos calculados arriba, escribe un breve resumen. 

**R. Con una confianza del 95% se obtiene que el estimador theta con bootstrap esta entre (.05047 y 1.0424), es decir es un testimador pequeño y si recordamos el cálculo del estimador :$$\theta_L = \frac{\lambda_2^{S}}{\lambda_2^{N}},$$

Donde  las $\lambda$s representan 2 cosas (dispersión y tambien de alguna manera considera el rank), entonces se puede concluir que para que este cociente sea un número pequeño puede ser por lo siguiente : numerador ($\lambda_2^{S}$) tiene un valor menor que el denominador ($\lambda_2^{N}$), en otras palabras $\lambda_2^{S}$ < $\lambda_2^{N}$, por lo que el grupo de tormetas que se somete a la aplicación del aerosol tiene menor dispersión que las que no tienen el aerosol. Esta conclusión es la que vemos en los histogramas de arriba (principalmente el de permitaciones), donde se ve que ambos grupos tienen distribuciones diferentes y la de las tormentas que tienen aplicado el aerosol esta bastante concentrada, casi no hay varianza, por el contrario las tormetas que no tienen aplicado este químico representan una distribución más aplanada con mayor varianza**
